...
reportRoute.get("/", authenticateToken(false), async (req, res) => {
	const {
		k = "",
		p = "1",
		category = "",
...
reportRoute.get("/self", authenticateToken(true), async (req, res) => {
	const {
		k = "",
		p = "1",
		category = "all",
...
reportRoute.get("/length", authenticateToken(false), async (req, res) => {
...
reportRoute.post("/", authenticateToken(true), upload.array('images', 5), async (req, res) => {
	if (!req.userRole) {
...
reportRoute.patch("/", authenticateToken(true), async (req, res) => {
	const { id } = req.query;
...
reportRoute.delete("/", authenticateToken(true), async (req, res) => {
	const { id } = req.query;
